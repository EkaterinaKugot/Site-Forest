    ...
    const op = {
        ...
        setChangeCss   : function (index, currentIndex, speed, easing, horizontal) {
            ...
            if (d.past==0) {
              ...
              deer4.parentElement.style.backgroundImage='linear-gradient(to bottom, white, rgb(0,0,0,0))'
              first_section.style.opacity='0'
            }
            if (d.now==1 && d.past==0)
              for (i=0; i<3; i++){
                numbers.children[i].style.animationName='vis'
                for (l=0; l<5; l++) numbers.children[i].children[l].style.animationName='vis_t'
                karuselkaid.style.animationName='vis_t'
              }
            if (d.now==0) {
              deer4.removeAttribute('style')
              deer4.parentElement.removeAttribute('style')
              first_section.removeAttribute('style')
            }
            if (d.now==0 && d.past==1){
              for (i=0; i<3; i++){
                numbers.children[i].removeAttribute('style')
                for (l=0; l<5; l++) numbers.children[i].children[l].removeAttribute('style')
                karuselkaid.removeAttribute('style')
              }
            }
            if (d.past==3) big_video.pause()
            if (d.now==4) {
              kalendar_sobitiy.getElementsByClassName('flexbox')[0].removeAttribute('style')
              kalendar_sobitiy.getElementsByClassName('kalendar')[0].removeAttribute('style')
            }
            if (d.past==4) {
              kalendar_sobitiy.getElementsByClassName('flexbox')[0].style.opacity='0'
              kalendar_sobitiy.getElementsByClassName('kalendar')[0].style.opacity='0'
              kalendar_sobitiy.getElementsByClassName('kalendar')[0].style.marginTop='10px'
            }
            if (d.now==5){
              kontakts.getElementsByClassName('way-hotel')[0].removeAttribute('style')
              for(i=1; i<5; i++) kontakts.children[i].style.animationName='vis'
            }
            if (d.past==5) {
              kontakts.getElementsByClassName('way-hotel')[0].style.opacity='0'
              kontakts.getElementsByClassName('way-hotel')[0].style.marginTop='10px'
              for(i=1; i<5; i++) kontakts.children[i].removeAttribute('style')
            }
            ...
            if (d.now==3 && d.past==2) {
              if (window.innerWidth<=910) informasia_ob_otele.getElementsByTagName('IMG')[1].style.top='40%'
              else informasia_ob_otele.getElementsByClassName('olenenok')[0].style.left='65%'
              informasia_ob_otele.getElementsByTagName('IMG')[2].style.top='80%'
              style.transitionDuration = 4000 + 'ms'
            }
            else {
              if (d.now==2 && d.past!=1){
                if (window.innerWidth<=910) informasia_ob_otele.getElementsByTagName('IMG')[1].removeAttribute('style')
                else {
                  informasia_ob_otele.getElementsByClassName('olenenok')[0].removeAttribute('style')
                  informasia_ob_otele.getElementsByTagName('IMG')[0].style.opacity='100%'
                }
                informasia_ob_otele.getElementsByTagName('IMG')[2].removeAttribute('style')
                style.transitionDuration = 4000 + 'ms'
              }
              else {
                if (d.past==2){
                  for(i=0; i<2;i++) informasia_ob_otele.getElementsByTagName('IMG')[i].removeAttribute('style')
                  informasia_ob_otele.getElementsByTagName('IMG')[1].style.top='50%'
                  informasia_ob_otele.getElementsByTagName('IMG')[0].style.left='68%'
                  style.transitionDuration = speed + 'ms'
                }
                else{
                  if (d.now==2 && d.past<2) {
                    informasia_ob_otele.getElementsByTagName('IMG')[0].removeAttribute('style')
                    informasia_ob_otele.getElementsByTagName('IMG')[1].removeAttribute('style')
                    for(i=0; i<2;i++) informasia_ob_otele.getElementsByTagName('IMG')[i].style.opacity='100%'
                    style.transitionDuration = 2000 + 'ms'
                  }
                  else{
                    if (d.past==0) {style.transitionDuration = 3000 + 'ms'}
                    else style.transitionDuration = speed + 'ms'
                  }
                }
              }
            }
